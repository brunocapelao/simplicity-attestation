...
def _script_hash_from_script_pubkey(script_pubkey: str) -> str:
    """Return the u256 script hash (sha256 of scriptPubKey bytes) as hex."""
    import hashlib

    return hashlib.sha256(bytes.fromhex(script_pubkey)).hexdigest()
...
class SAS:
    ...
    def create_vault(
        ...
        # Compile certificate first so we can compute its script hash for vault covenants.
        ...
        cert_script_hash = _script_hash_from_script_pubkey(cert_script_pubkey)

        # Update contracts with pubkeys (+ cert hash) and compile
        vault_data = cls._compile_contract_with_keys(
            ...
            hal_path,
            network,
            cert_script_hash=cert_script_hash,
        )
    ...
    def _build_and_sign_issue(self, vault_utxo: UTXO, cid: str) -> TransactionResult:
        ...
        try:
            ...
            if not run_result.sig_all_hash:
                raise SAPError("Unable to extract sig_all_hash for vault spend (hal-simplicity pset run)")
    ...
    def _build_and_sign_revoke(
        ...
        try:
            ...
            if not run_result.sig_all_hash:
                raise SAPError("Unable to extract sig_all_hash for certificate spend (hal-simplicity pset run)")
    ...
    def _build_and_sign_drain(self, vault_utxo: UTXO, recipient: str) -> TransactionResult:
        ...
        try:
            ...
            run_result = self.hal.pset_run(pset, 0, self.config.vault_program, dummy_witness)
            if not run_result.sig_all_hash:
                raise SAPError("Unable to extract sig_all_hash for vault drain (hal-simplicity pset run)")
    ...
    def _compile_contract_with_keys(
        ...
        hal_path: str,
        network: str,
        cert_script_hash: Optional[str] = None,
    ) -> dict:
        ...
        if template_path.name == "vault.simf":
            if not cert_script_hash or len(cert_script_hash) != 64:
                raise CompilationError("vault.simf requires cert_script_hash (64 hex chars)")
            cert_hash_pattern = r'(cert_script_hash:\s*u256\s*=\s*0x)([a-f0-9]{64})'
            content, count = re.subn(
                cert_hash_pattern,
                r'\g<1>' + cert_script_hash,
                content,
            )
            if count != 1:
                raise CompilationError("Unable to patch cert_script_hash in vault.simf")
