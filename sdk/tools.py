...
class ToolManager:
    ...
    def get_path(self, tool_name: str) -> Path:
        ...
        if not binary_path.exists():
            ...
            raise ToolError(
                f"Tool '{tool_name}' not installed and not found in PATH. "
                f"Run: ToolManager().install('{tool_name}') or install manually."
            )
    ...
    def status(self) -> Dict[str, dict]:
        ...
        for name, info in TOOLS.items():
            ...
            if installed:
                try:
                    resolved_path = str(self.get_path(name))
                except ToolError:
                    resolved_path = None
            result[name] = {
                "installed": installed,
    ...
    def install(self, tool_name: str, force: bool = False) -> Path:
        ...
        if not url:
            raise ToolError(
                f"No download URL is configured for '{tool_name}'. "
                f"Install it manually and ensure it's in PATH."
            )
